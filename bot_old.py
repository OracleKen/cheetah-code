        ...
        if states["status"] == "inCity":
            ...
            if states["multiCharacterMode"]:
                if sum(states["multiCharacterModeState"]) == 0:
                    ...
                    # hestera
                    print("checking hestera")
                    print("config enableHestera: {}".format(config["enableHestera"]))
                    print("character hestera: {}".format(config["characters"][states["currentCharacter"]]["hestera"]))
                    if (
                            config["enableHestera"]
                            and config["characters"][states["currentCharacter"]]["hestera"]
                    ):
                        # do hestera
                        print("doing hestera on : {}".format(states["currentCharacter"]))
                        doHestera()
                        sleep(1400, 1600)
...
def checkCDandCast(ability):
    ...
    #     sleep(50, 110)
        ...
        pydirectinput.press("z")
        sleep(110, 200)
    elif pyautogui.locateOnScreen("./screenshots/classes/" + className + ".png", region=config["regions"]["specials"], confidence=.7):
        print("special found")
        #mouse.click(Button.x2)
        pydirectinput.press("z")
        sleep(110, 200)
...
def checkFloor2Boss():
    ...
    bossLocation = pyautogui.locateCenterOnScreen(
        "./screenshots/boss.png", confidence=0.6, region=config["regions"]["minimap"]
    )
    ...
    if config["characters"][states["currentCharacter"]]["class"] == "striker":
        for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            if ability["abilityType"] == "awakening":
                continue
            im = "./screenshots/striker/" + ability["key"] + ".png"
        ...
        for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            if ability["abilityType"] == "awakening":
                continue
            im = "./screenshots/scrapper/" + ability["key"] + ".png"
            states["abilityScreenshots"].append(
                {
                    "key": ability["key"],
                    "image": im,
        ...
        for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            ...
            sleep(200, 300)
    if config["characters"][states["currentCharacter"]]["class"] == "shadowhunter":
        for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            if ability["abilityType"] == "awakening":
                continue
            im = "./screenshots/shadowhunter/" + ability["key"] + ".png"
            states["abilityScreenshots"].append(
                {
                    "key": ability["key"],
                    "image": im,
