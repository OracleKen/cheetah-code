        ...
        if states["status"] == "inCity":
            ...
            if states["multiCharacterMode"]:
                if sum(states["multiCharacterModeState"]) == 0:
                    ...
                    if(states["currentCharacter"] == config["mainCharacter"] and 
                       config["characters"][states["currentCharacter"]].get("goHome") == True):
                        sendToStronghold()
                        sleep(1200,1400)
...
def goOffline():
    ...
    pyautogui.click(button="left")
    sleep(400, 500)
    mouseMoveTo(x=config["offlinePositions"][1][0], y=config["offlinePositions"][1][1])
    sleep(400, 500)
    pyautogui.click(button="left")
    sleep(400, 500)
    mouseMoveTo(x=config["offlinePositions"][2][0], y=config["offlinePositions"][2][1])
    sleep(400, 500)
    pyautogui.click(button="left")
    sleep(400, 500)
    mouseMoveTo(x=config["offlinePositions"][3][0], y=config["offlinePositions"][3][1])
    sleep(400, 500)
    pyautogui.click(button="left")
    sleep(400, 500)
...
def enterChaos():
    ...
    if config["shortcutEnterChaos"] == True:
        ...
        while True:
            ...
            if ((aor != None and config["performance"] == False) or config["forceAorMode"]):
                states["floor3Mode"] = True
                print("aor detected")
                if (
            ...
            chaosTabPosition = {
                ...
                # south vern
                1415: [[1065, 307], [524, 400]],
                1445: [[1065, 307], [524, 455]],
                1475: [[1065, 307], [524, 505]],
                1490: [[1065, 307], [524, 555]],
                1520: [[1065, 307], [524, 605]],
                1540: [[1065, 307], [524, 662]],
                1560: [[1065, 307], [524, 715]],
                #elgacia
...
def doFloor1():
    ...
    # className = config["characters"][states["currentCharacter"]]["class"]
    ...
    if(config["characters"][states["currentCharacter"]].get("awakeningAlways") == True):
        useAwakening()
...
def doFloor2():
    ...
    while True:
        ...
        #print("state[status] = " + states["status"])
        # 不小心进下个门但没识别到
        ...
        print("cleared 2 and moving to portal")
        calculateMinimapRelative(states["moveToX"], states["moveToY"])
...
def checkCDandCast(ability):
    #print("checkCDandCast function entered")
    className = config["characters"][states["currentCharacter"]]["class"]
    if className == "arcana":
        ...
        sleep(110, 200)
    # elif className == "slayer" and pyautogui.locateOnScreen("./screenshots/classes/" + className + "_rdy.png", region=config["regions"]["specials"], confidence=.6): 
    #     pydirectinput.press("z")
    #     sleep(50, 110)
    # elif className == "scouter":
    #     # pydirectinput.press("x")
    #     sleep(110,200)
    elif className == "artist" and pyautogui.locateOnScreen("./screenshots/classes/" + className + ".png", region=config["regions"]["specials"], confidence=.6):
        mouseMoveTo(x=config["screenCenterX"], y=config["screenCenterY"])
        sleep(50, 100)
        pydirectinput.press("x")
        sleep(1000, 1200)
        ...
        pydirectinput.press("z")
        sleep(50, 110)
    elif className == "berserker" and pyautogui.locateOnScreen("./screenshots/classes/newzerk.png", region=config["regions"]["specials"], confidence=.7):
        print("berserker mayhem found")
        pydirectinput.press("z")
        sleep(110, 200)
    elif pyautogui.locateOnScreen("./screenshots/classes/" + className + ".png", region=config["regions"]["specials"], confidence=.6):
        #print("special found")
        #mouse.click(Button.x2)
        pydirectinput.press("z")
        sleep(110, 200)
        if className == "deathblade":
            sleep(150, 250)
            #mouse.click(Button.x2)
            pydirectinput.press("z")
            print("surge!")
        # elif className == "shadowhunter":
        #     sleep(250, 350)
        #     saveAbilitiesScreenshotsDemon()
    if (
            config["performance"] == True
            or config["GFN"] == True
        ...
        diedCheck()
        castAbility(ability, config["regions"]["abilities"])

def castAbility(ability, region): 
    if ability["directional"] == True:
            shortCast = ability["shortCast"]
            if shortCast == None: 
                shortCast = False

            #print("shortCast value: {}".format(shortCast))
            if shortCast:
                newX = config["screenCenterX"]
                newY = config["screenCenterY"]
                if states["moveToX"] - newX >= 100: 
                    newX = newX + 75
                elif states["moveToX"] - newX > 0: 
                    newX = newX + 20
                elif states["moveToX"] - newX <= -100: 
                    newX = newX - 75
                elif states["moveToX"] - newX < 0: 
                    newX = newX - 20

                if states["moveToY"] - newY >= 100: 
                    newY = newY + 75
                elif states["moveToY"] - newY > 0: 
                    newY = newY + 20
                elif states["moveToY"] - newY <= -100: 
                    newY = newY - 75
                elif states["moveToY"] - newY < 0: 
                    newY = newY - 20

                # print("shortCast: centerX: {}, centerY: {}, moveToX: {}, moveToY: {}, newX: {}, newY: {}".format(config["screenCenterX"], config["screenCenterY"], states["moveToX"], states["moveToY"], newX, newY))
                mouseMoveTo(x=newX, y=newY)              
            else: 
...
def checkFloor2Boss():
    ...
    bossLocation = pyautogui.locateCenterOnScreen(
        "./screenshots/boss.png", confidence=0.55, region=config["regions"]["minimap"]
    )
...
def saveStaticAbilitiesScreenshots(className): 
    for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            ...
            if(ability.get("staticImage") == True):
                im = "./screenshots/" + className + "/" + ability["key"] + ".png"
            else:
...
def saveAbilitiesScreenshots():
    if config["characters"][states["currentCharacter"]]["class"] == "striker":
        saveStaticAbilitiesScreenshots("striker")
    elif config["characters"][states["currentCharacter"]]["class"] == "scrapper":
        saveStaticAbilitiesScreenshots("scrapper")
    elif config["characters"][states["currentCharacter"]]["class"] == "shadowhunter":
        saveStaticAbilitiesScreenshots("shadowhunter")
    elif config["characters"][states["currentCharacter"]]["class"] == "souleater":
        saveStaticAbilitiesScreenshots("souleater")
    else:
        for ability in abilities[config["characters"][states["currentCharacter"]]["class"]]:
            left = ability["position"]["left"]
            top = ability["position"]["top"]
            ...
            states["abilityScreenshots"].append(
                {
                    ...
                    "holdTime": ability["holdTime"],
                    "directional": ability["directional"],
                    "shortCast": ability.get("shortCast")
                }
            )
            sleep(200, 300)
...
def healthCheck():
    if config["useHealthPot"] == False and (config["useHealthPotAor"] == False or states["multiCharacterMode"] == False or states["multiCharacterModeState"][states["currentCharacter"]] <= 0):
        return
    x = int(
        config["healthCheckX"]
...
def checkTimeout():
    ...
    if (
        ...
        timeout = pyautogui.screenshot()
        timeout.save("./debug/timeout_" + now + ".png")
        states["timeoutCount"] = states["timeoutCount"] + 1
        return True
    elif (
            states["floor3Mode"] == True
        ...
        timeout = pyautogui.screenshot()
        timeout.save("./debug/timeout_aor_" + now + ".png")
        states["timeoutCount"] = states["timeoutCount"] + 1
        return True
    return False
...
def gameCrashCheck():
    ...
    if sum > 10:
        ...
        crash.save("./debug/crash_" + now + ".png")
...
def restartGame():
    ...
    print("starting outer restart loop")
    while True:
        exitGame = pyautogui.locateCenterOnScreen(
            "./screenshots/exitGame.png", confidence=0.75
        )
        sleep(500,600)
        enterGame = pyautogui.locateCenterOnScreen(
            "./screenshots/steamPlay2.png", confidence=0.65
        )
        sleep(500, 600)
        ...
        sleep(500,600)
        if exitGame != None: 
            print("clicking exit game")
            mouseMoveTo(x=950, y=565)
            sleep(500,600)
            pydirectinput.click(x=950, y=565, button="left")
            sleep(500,600)
        elif stopGame != None:
            ...
            if confirm == None:
                print("steam confirm none - continuing")
                continue
            x, y = confirm
            mouseMoveTo(x=x, y=y)
            sleep(1200, 1300)
            ...
            print("breaking for eacoffline interface")
            # new eacoffline interface
            break
        elif leaveButton != None: 
            print("found leaveButton - quitting chaos")
            quitChaos()
            return
        elif inTown != None:
            print("restart found inTown")
            return
            ...
            #     continue
        sleep(1200, 1300)
    print("passing outer restart while loop")
    sleep(5200, 6300)
    print("entering 2nd restart while")
    while True:
        enterServer = pyautogui.locateCenterOnScreen(
            "./screenshots/enterServer.png",
            confidence=config["confidenceForGFN"],
...
def switchToCharacter(index):
    ...
    mouseMoveTo(x=1890, y=910)
    sleep(200,300)
    pydirectinput.click(button="left")
    sleep(200,300)
    mouseMoveTo(x=1840,y=840)
    sleep(200,300)
    pydirectinput.click(button="left")
    sleep(1000, 1200)
...
def doGuildDonation():
    ...
    # dono silver
    silverDono = config["characters"][states["currentCharacter"]].get("guildDonateSilver")
    if(silverDono == True):
...
def doHestera():
    sleep(1000, 2000)
    print("accepting Hestera daily")
    doDaily = acceptHesteraDaily()
    sleep(1500, 1600)
    if doDaily == False:
        return
    sleep(500, 600)
    ...
    # goto hestera
    bifrostAvailable = bifrostGoTo(config["characters"][states["currentCharacter"]]["hesteraSpot"])
    if bifrostAvailable == False:
        return
    if gameCrashCheck():
        return
...
def useAwakening(): 
    ...
    region = config["regions"]["awakening"]
    if(ability != None and ability.get("image") != None):
        if(pyautogui.locateOnScreen(ability["image"], region=region)):
            castAbility(ability, region)
    else: 
        #default to a vanilla awakening attempt.
        castAbility(
            {
                "key": config["awakening"],
                "cast": False,
                "castTime": None,
                "hold": False,
                "holdTime": None, 
